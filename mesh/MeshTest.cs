    ...
    public class MeshTest : IDisposable
    {
        ...
        public async Task<string> SetupTest(string useregister, int threads)
        {
            ...
            for (int i = 0; i < threads; i++)
            {
                ...
                testParticipants.Add(newWallet);
                Console.WriteLine($"Created new Wallet[{i}] : {testParticipants[i].Address}");
            }

            var blueprint = BuildBlueprint("pingpong/pingpong.json", testParticipants);
            ...
            if (testParticipants.Count < 1)
            {
                throw new Exception("Must run a number of threads/wallets");
            }
            bpTxId = await _blueprintServiceClient.PublishBlueprint(testParticipants[0].Address, registerId, blueprint);
